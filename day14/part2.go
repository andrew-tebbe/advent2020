...
func main() {
	...
	for _, section := range initialization {
		...
		for _, operation := range section.operations {
			applyMask(operation.address, operation.setValue, mask)
		}
	}
	sum := 0
...
func applyMask(address int, memVal int, mask []int) {
	addresses := []int{0}
	for i := len(mask) - 1; i >= 0; i-- {
		bitPos := 1 << i
		idxVal := 19
		if mask[len(mask)-i-1] == Zero {
			idxVal = address & bitPos
		}
		if mask[len(mask)-i-1] == One {
			idxVal = bitPos
		}
		if mask[len(mask)-i-1] == Float {
			curLen := len(addresses)
			for j := 0; j < curLen; j++ {
				addresses = append(addresses, addresses[j]|bitPos)
			}
		} else {
